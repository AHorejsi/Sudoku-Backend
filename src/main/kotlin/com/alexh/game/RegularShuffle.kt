...
fun shuffleBoardOfRegular(puzzle: RegularSudoku, rand: Random) {
    val shuffleTypes = mutableListOf<(RegularSudoku, Random) -> Unit>(::inner, ::flip, ::flipBox, ::rotate)
    val runs = 5

    for (count in 0 until runs) {
    ...
    if (!puzzle.valid) {
        throw InternalError("Puzzle must be valid by this point\n\n$puzzle")
    }
}
